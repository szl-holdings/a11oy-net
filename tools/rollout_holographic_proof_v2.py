...
Documents with a zero-JavaScript contract receive a CSS-only static journey
rail. Other documents receive the interactive first-party controller. The
operation is deterministic, idempotent, and preserves all evidence content.
"""
...
def remove_script(text: str) -> tuple[str, bool]:
    original = text
    pattern = re.compile(
...
def adopt_existing_static_rail(text: str) -> tuple[str, bool]:
    pattern = re.compile(
...
def is_bound(relative: str, text: str) -> bool:
    ...
    if relative in ZERO_JAVASCRIPT:
        return SCRIPT not in text and (
            ADOPTED_MARKER in text or STATIC_MARKER in text
        )
...
def bind(path: Path) -> str:
    ...
    for marker in (
        ...
        'data-szl-proof-holo-asset="script-v2"',
        STATIC_MARKER,
    ):
    ...
    if relative in ZERO_JAVASCRIPT:
        text, removed = remove_script(text)
        changed = changed or removed
        text, adopted = adopt_existing_static_rail(text)
...
def run(*, check: bool) -> dict[str, object]:
    ...
    for path in documents():
        relative = path.relative_to(ROOT).as_posix()
        if check:
            result = "present" if is_bound(relative, read(path)) else "missing"
        else:
            result = bind(path)
        rows.append(
            {
                "path": relative,
                "result": result,
                "mode": "STATIC" if relative in ZERO_JAVASCRIPT else "INTERACTIVE",
            }
        )
