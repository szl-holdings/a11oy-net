...
- RECORD is a static, fetchable receipt-record **index** (pointers, not a receipt database). Interactive `/verify` remains `https://a-11-oy.com/verify` and is not cloned here. Empty receipt-id listing is `UNAVAILABLE`, never invented IDs.
- `/health.json` is the committed static probe document. It is not runtime health, not DSSE-LIVE, and not uptime. `/readyz` is an HTML directory (Pages may 301 `/readyz` → `/readyz/`) and is not a JSON health URL.
