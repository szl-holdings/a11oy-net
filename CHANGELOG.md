...
- `/health.json` is the only health document: a committed static JSON file. It is not runtime health, not DSSE-LIVE, and not uptime. `/readyz` is an HTML directory (Pages may 301 `/readyz` → `/readyz/`) and is not a health URL. `/healthz` is not published (404 HTML) and is not a probe.
