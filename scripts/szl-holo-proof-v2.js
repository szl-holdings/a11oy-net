 ...
 * Interactive enhancement only. Zero-JavaScript proof documents use the
 * equivalent static rail installed by the source binder.
...
(() => {
  ...
  const ROUTES = [
    ["/record", "RECORD", "Proof"],
    ["/diligence", "Diligence Room", "Proof"],
    ["/estate", "Estate Atlas", "Models"],
    ["/atelier", "Atelier Record", "Models"],
    ["/khipu", "KHIPU Record", "Kernels"],
    ["/decision", "Decision Record", "Products"],
    ["/security", "Security Record", "Proof"],
    ["/notes", "Research Notes", "Proof"],
    ["/origin", "Origin Record", "Proof"],
    ["/vessels", "Vessels Record", "Products"],
    ["/terra", "Terra Record", "Products"],
    ["/aegis", "Aegis Record", "Products"],
    ["/counsel", "Counsel Record", "Products"],
    ["/factory", "Factory Record", "Products"],
    ["/frontiers", "Frontiers Record", "Proof"],
    ["/", "A11oy Proof Network", "Start"],
  ];
  ...
  function addSkipLink() {
    ...
    document.body.prepend(createElement("a", {
      className: "szl-proof-holo-skip",
      href: `#${main.id}`,
      dataset: { szlProofHoloSkip: "true" },
    }, "Skip to main content"));
  }

  function adoptExistingRail() {
    const existing = document.querySelector(".szl-proof-rail");
  ...
  function buildRail(currentRoute) {
    ...
    const rail = createElement("header", {
      className: "szl-proof-holo-rail",
      dataset: { szlProofHoloRail: "v2" },
    });
    const identity = createElement("a", {
      className: "szl-proof-holo-identity",
      href: `${PROOF}/`,
      "aria-label": "Open the A11oy Proof Network",
    });
    identity.append(createElement("span", { className: "szl-proof-holo-mark", "aria-hidden": "true" }));
    const copy = createElement("span", { className: "szl-proof-holo-copy" });
    copy.append(createElement("span", { className: "szl-proof-holo-eyebrow" }, "SZL · Evidence Vault"));
    copy.append(createElement("span", { className: "szl-proof-holo-label" }, currentRoute.label));
    identity.append(copy);

    const controls = createElement("div", { className: "szl-proof-holo-controls" });
    const menu = createElement("button", {
      className: "szl-proof-holo-menu",
      type: "button",
      "aria-label": "Open proof navigation",
      "aria-expanded": "false",
      "aria-controls": "szl-proof-holo-nav",
    }, "Menu");
    const nav = createElement("nav", {
      className: "szl-proof-holo-nav",
      id: "szl-proof-holo-nav",
      "aria-label": "A11oy proof journeys",
      dataset: { open: "false" },
    });
  ...
  function addAmbientAndProgress() {
    if (!document.getElementById("szl-proof-holo-ambient")) {
      document.body.prepend(createElement("div", {
        id: "szl-proof-holo-ambient",
        "aria-hidden": "true",
        dataset: { szlProofHoloDecorative: "true" },
      }));
    }
    if (!document.querySelector(".szl-proof-holo-progress")) {
      document.body.append(createElement("div", {
        className: "szl-proof-holo-progress",
        "aria-hidden": "true",
        dataset: { szlProofHoloDecorative: "true" },
      }));
    }
  ...
  function installMotion() {
    ...
    const commitScroll = () => {
      ...
      root.style.setProperty("--szl-proof-v2-progress", percentage.toFixed(2));
    };
    const scroll = () => {
  ...
  function boot() {
    ...
    addAmbientAndProgress();
    addSkipLink();
    buildRail(currentRoute);
    enhancePanels();
    installMotion();

    window.SZLProofHolo = Object.freeze({
      version: VERSION,
      route: Object.freeze(currentRoute),
      decorativeMotion: true,
      measuredTelemetry: false,
      zeroJavascriptDocumentsSupported: true,
    });
    document.dispatchEvent(new CustomEvent("szl:proof-holo-ready", {
      detail: { version: VERSION, route: currentRoute.prefix, label: currentRoute.label },
    }));
  }
