...
"""S7 fail-closed: every a11oy.net kernel chip binds /honest (8 or N/A).

INTI owns the bind. Keep RED until every kernel chip binds /honest.
Catalog genome LOCKED-PROVEN=25 stays labelled. Do not demand 25 be deleted.
...
def test_every_kernel_chip_binds_honest():
    ...
    assert not failures, (
        ...
        "not genome LOCKED-PROVEN=25. Both numbers are real. Catalog 25 stays "
        f"labelled. Do not demand 25 be deleted. {failures} {verdict.detail}"
    )
